...
const (
	...
	ES_URL      = "http://10.128.0.7:9200"
	BUCKET_NAME = "lan-bucket-around"
)
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// save image to GCS
	// header -- properties of the file(date, size, ...)
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Failed to read post into JSON format", http.StatusInternalServerError)
...
func saveToES(i interface{}, index string, id string) error {
	...
	_, err = client.Index(). // preparedstatement = ...
		// index -- refer to which database
		Index(index). //statement.set(1, userid)
		Id(id).
		BodyJson(i).
		// do -- excute
		Do(context.Background()) //statement.excuteUpdate
